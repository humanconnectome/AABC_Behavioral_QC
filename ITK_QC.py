...
## some of this is preconfigured.  Others aren't.  For ITK its more messy.
PRCfolder='340425049927'
#better logic than this DNR list is used when creating a clean dataset...this is just a fly-by
...
freeze2file="Union_HCA-AABC_Data-Freeze-2_id-events_2025-09-15.csv"
...
#AABCdict=pd.read_csv(box.download(config['config_files'][aabc_redcap_dict]),low_memory=False)
...
#dont run fipscheck here...run as specialty QC check in adicalc.py
...
#not sure if this is doing what we think its doing...
inventoryaabc3.loc[(inventoryaabc3.q_unusable=='1')]
...
#FIRST read it in see #1 above for how they were generated.
...
for i in list(nightmares.PIN):
    print(dffull.loc[dffull.PIN==i][['PIN','DateFinished','App Version']].drop_duplicates(subset='PIN',keep='first'))
for i in list(nightmares.PIN):
...
#do this if you're ready for PRC and STG
#box.upload_file("Upload_Processing_TLBX_RAW.csv",PRCfolder)
#box.upload_file("Upload_Processing_TLBX_Scores.csv",PRCfolder)
#box.upload_file("Upload_Processing_TLBX_Registration.csv",PRCfolder)

# %%
# proceed to generate tickets using the scores part
...
### NOW For ASA 24 ######################################################################
# ORDER
# 1. scan for data run asa24scan.py ahead of time.  it takes a while (here just looking for existende)
...
#sometimes you can just skip the concatenation by sites... better to run asa24scan.py by site though because REALLY LONG
...
#just QC'ing the totals since that is derived from the other datatypes
...
#moved scan to actiscan_wCAMNTech.py which creates temp_actigraphy.csv
...
cobramergeFreeze=pd.read_csv("/Users/petralenzini/work/Behavioral/AABC/AABC_Behavioral_QC/AABC_Behavioral_QC/"+freeze2file).drop(columns='Site')
...
cobramergeFreezeforCobraAABC=cobramergeFreezeforCobra.loc[~(cobramergeFreezeforCobra.study=='HCA')].drop(columns=['race','age_visit','study','M/F','ethnic_group','age_bin'])
...
reorder=['subject','redcap_event','Site','event_date','Freeze','watch','Cobra_Scored','AnyActigraphyDataCollected','actigraphy_upload_typo','FailedScreen','priority']
